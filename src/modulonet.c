...
void load_pretrained_model(int16_t w0[784][4096], int16_t w1[4096][4096], int16_t w2[4096][4096], int16_t w3[4096][10], int16_t b0[4096], int16_t b1[4096], int16_t b2[4096], int16_t b3[10]){
	...
	fclose(w0_f);
	FILE *w1_f = fopen("../trained_model/l1_w.txt", "r");
	if(w1_f == NULL){
		printf("Error opening l1_w.txt\r\n");
	}
	for (uint32_t i = 0; i < 4096; i++){
	...
	fclose(w1_f);
	FILE *w2_f = fopen("../trained_model/l2_w.txt", "r");
	if(w2_f == NULL){
		printf("Error opening l2_w.txt\r\n");
	}
	for (uint32_t i = 0; i < 4096; i++){
		for (uint32_t j = 0; j < 4096; j++){
	...
	fclose(w2_f);
	FILE *w3_f = fopen("../trained_model/l3_w.txt", "r");
	if(w3_f == NULL){
		printf("Error opening l3_w.txt\r\n");
	}
	...
	fclose(w3_f);
	FILE *biases_fptr = fopen("../trained_model/biases.txt", "r");
	if(biases_fptr == NULL){
		printf("Error opening biases.txt\r\n");
	}
	for (uint32_t i = 0; i < 4096; i++){
